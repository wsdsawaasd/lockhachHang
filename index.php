...
function searchByDate($customers, $from_date, $to_date)
{
    ...
    foreach ($customers as $customer) {
        if ((strtotime($customer['day_of_birth']) >= strtotime($from_date) && (strtotime($customer['day_of_birth']) <= strtotime($to_date)))) {
...
<table border="0">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        echo strtotime($from_date);
        $to_date = $_POST["to"];
        echo "&nbsp;&nbsp;";
        echo strtotime($to_date);
    }
